...
from module.layers import SeqPoolingLayer, MLPLayers
...
class SASRec9(BaseModel):
    ...
    def training_step(self, batch, reduce=True, return_query=False, align=False):
        loss_value, query =  super().training_step(batch, reduce=False, return_query=True)
        score = torch.sigmoid(self.scorer(query)).squeeze() # BL
        loss_value = (loss_value * score).sum()
        return loss_value