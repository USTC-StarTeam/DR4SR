...
class Student(BaseModel):
    def __init__(self, config, dataset_list : list[dataset.BaseDataset]) -> None:
        ...
        sub_model_config['model'] = 'SASRec2'
    ...
    def _init_model(self):
        ...
        nn.init.normal_(self.synthetic_dataset.weight)
        ...
        self.experts = torch.load('saved/Teacher/amazon-toys/2023-12-13-21-26-16-623139-timestamp.pth', map_location='cpu')
        ...
        for outer_idx in range(50):
            ...
            for _ in range(1):
                for these_indices in indices_chunks:
                    ...
                    neg_idx = neg_idx.reshape(query.shape[0], self.max_seq_len, self.config['num_neg'])
                    neg_score = (query.unsqueeze(-2) * self.item_embedding.weight[neg_idx]).sum(-1)
                    loss_value = self.loss_fn(pos_score, neg_score)

                    grad = torch.autograd.grad(loss_value, student_params[-1], create_graph=True)[0]
                    student_params.append(student_params[-1] - 50 * grad)
                    
                    # self.optimizer.zero_grad()
                    # loss_value.backward()
                    # self.optimizer.step()

                    # student_params.append(self._remove_meta_parameters(self.state_dict()))
                random.shuffle(indices_chunks)
            ...
            #     torch.cat([p.flatten() for p in self.parameters()][:len(target_params)]),
            #     flatten_state_dict(target_params),
            #     reduction="sum"
            # )
            # param_dist += torch.nn.functional.mse_loss(
            #     flatten_state_dict(starting_params),
            #     flatten_state_dict(target_params),
            #     reduction="sum"
            # )
