...
class SASRec5(BaseModel):
    def __init__(self, config, dataset_list : list[dataset.BaseDataset]) -> None:
        ...
        self.query_encoder_twin = SASRecQueryEncoder(
            self.fiid,
            self.embed_dim,
            self.max_seq_len,
            config['model']['head_num'],
        ...
        self.condition_proj = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.embed_dim, self.L * self.embed_dim),
        )
        # self.condition_proj = nn.Linear(self.embed_dim, self.L * self.embed_dim)
    ...
    def forward(self, batch, need_pooling=True):
        ...
        # query_c = query.reshape(B, 1, D)
        # self.query_encoder_twin.load_state_dict(self.query_encoder.state_dict())
        batch = {
            'seq_emb': query_c,
            'seqlen': self.L * torch.ones(B, dtype=torch.int64, device=self.device),
        }
